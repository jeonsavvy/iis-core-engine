...
@pytest.mark.asyncio
async def test_agent_loop_retries_specialization_before_accepting_vertex_result() -> None:
    codegen = StubCodegen(
        results=[
            CodegenResult(html="<html><body>terrain demo only</body></html>", generation_source="vertex", model_name="gemini"),
            CodegenResult(
                html="<html><body>checkpointstate laptimer throttle brake requestAnimationFrame</body></html>",
    ...
    visual = StubVisualQA(results=[VisualQAResult(ok=True, score=0, feedback="visual fine", issues=[])] * 3)
    playtester = StubPlaytester(
        results=[PlaytestResult(boots_ok=True, has_errors=False, issues=[], fatal_issues=[], feedback="ok", score=0)] * 3
    )
    ...
    assert result.html == "<html><body>checkpointstate laptimer throttle brake requestAnimationFrame</body></html>"
    assert result.reverted_to_baseline is False
    assert len(codegen.prompts) >= 2
    assert not any(activity.error_code == "scaffold_specialization_rejected" for activity in result.activities)


@pytest.mark.asyncio
async def test_agent_loop_retries_flight_specialization_before_accepting_vertex_result() -> None:
    codegen = StubCodegen(
        results=[
            CodegenResult(html="<html><body>autoscroll corridor shooter</body></html>", generation_source="vertex", model_name="gemini"),
            CodegenResult(
                html="<html><body>reticle enemylasers boostcharge requestAnimationFrame pitch roll yaw throttle</body></html>",
                generation_source="vertex",
                model_name="gemini",
            ),
            CodegenResult(
    ...
    assert result.html == "<html><body>reticle enemylasers boostcharge requestAnimationFrame pitch roll yaw throttle</body></html>"
    assert result.reverted_to_baseline is False


@pytest.mark.asyncio
async def test_agent_loop_keeps_last_playable_specialization_when_contract_drift_remains() -> None:
    codegen = StubCodegen(
        results=[
            CodegenResult(html="<html><body>clicker 8-way shooter</body></html>", generation_source="vertex", model_name="gemini"),
            CodegenResult(
                html="<html><body>phaser dash firebullet requestAnimationFrame crosshair enemybullets</body></html>",
                generation_source="vertex",
                model_name="gemini",
            ),
            CodegenResult(
    ...
    assert result.html == "<html><body>phaser dash firebullet requestAnimationFrame crosshair enemybullets</body></html>"
    assert result.reverted_to_baseline is False


@pytest.mark.asyncio
async def test_agent_loop_reverts_to_scaffold_only_when_specialization_is_fatal() -> None:
    codegen = StubCodegen(
        results=[
            CodegenResult(html="<html><body>terrain demo only</body></html>", generation_source="vertex", model_name="gemini"),
            CodegenResult(html="<html><body>terrain demo only</body></html>", generation_source="vertex", model_name="gemini"),
