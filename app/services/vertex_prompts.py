...
def build_codegen_prompt(
    *,
    ...
    return (
        ...
        "- Framework contract is strict: if request/runtime is 3D use Three.js, if 2D use Phaser.js.\n"
        "- If the framework contract is violated, output is rejected.\n"
        ...
        "- Keep primitive fillRect-only rendering under 35% of total draw operations; use paths/gradients/sprites for the rest.\n"
        "- Provide at least 3 distinct visual layers (foreground gameplay, readable midground, animated background/postfx).\n"
        "- Keep at least 4 distinguishable silhouette types for player/enemy/hazard/interactive object.\n"
        "- Movement and steering must remain analog/time-based; avoid one-step quantized control.\n"
        "- Preserve/expand encounter systems: enemy pattern tables, wave escalation, miniboss events.\n"
        ...
        "- Do not render generic genre labels like `Genre: arcade` in HUD text.\n"
        "- HUD must stay concise. Never expose debug/meta jargon such as Lv/W/XP/Relic/Syn/Build.\n"
        "- Do not output markdown fences.\n\n"
        "=== EXPECTED QUALITY BAR (AA-grade reference) ===\n"
        "Your output MUST meet or exceed this quality level. Games below this bar will be REJECTED:\n"
        "1. RENDERING: Use custom GLSL shaders (minimum 3: e.g., atmospheric scattering, volumetric effects, noise-based clouds). "
        "Never rely solely on Canvas 2D fillRect. Use THREE.ShaderMaterial with custom vertexShader/fragmentShader.\n"
        "2. WORLD: Implement procedural terrain/environment using noise functions (simplex/perlin fbm). "
        "Use LOD (Level of Detail) techniques like QuadTree subdivision for large terrain. Minimum 800+ lines of game code.\n"
        "3. OBJECT POOLING: Use InstancedMesh or object pools for particles, trails, enemies. "
        "Minimum pool sizes: 100+ instances for visual effects, 10+ for active entities.\n"
        "4. AI: Implement NPC/enemy FSM (Finite State Machine) with at least 3 states (e.g., patrol/chase/attack or orbit/deepspace/dying). "
        "Include obstacle avoidance and target acquisition behaviors.\n"
        "5. STATE MACHINE: Game must have at least 3 player states (e.g., FLIGHT/LANDED/DEAD or PLAYING/MENU/GAMEOVER). "
        "Each state must have distinct controls, camera, and HUD.\n"
        "6. PHYSICS: Implement meaningful physics (gravity, collision response, momentum). "
        "Use delta-time based updates. Include terrain normal alignment for ground contact.\n"
        "7. CAMERA: Implement at least 2 camera modes (e.g., third-person chase + spectator, or FPS + orbital). "
        "Camera must smooth-follow with slerp/lerp interpolation.\n"
        "8. EFFECTS: Implement particle/trail systems, hit effects with additive blending, "
        "dynamic point lights, and screen-space post-processing (fog, bloom-like glow).\n"
        "9. MINIMUM COMPLEXITY: At least 15 named functions, 800+ total lines, "
        "3 custom GLSL shaders, and 2+ interactive mechanics.\n"
        "=== END QUALITY BAR ===\n\n"
        f"Keyword: {keyword}\n"
        f"Title: {title}\n"
        f"Genre: {genre}\n"
