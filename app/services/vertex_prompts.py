...
def build_codegen_prompt(
    *,
    ...
    return (
        ...
        "- Do not reference add-on classes through `THREE.<Symbol>` namespace (Controls/Pass/Composer/Loader/Utils).\n"
        "- If an add-on constructor is used, it must be explicitly imported in the same HTML before usage.\n"
        ...
        "- Rendering contract is mandatory (do not substitute with plain rectangles):\n"
        "  1) At least one animated gradient layer.\n"
        "  2) At least one stroke/path-based silhouette layer.\n"
        "  3) At least one particle or trail motion layer.\n"
        "  4) At least one depth/parallax layer that moves over time.\n"
        "  5) At least 8 distinct color buckets actively used in scene rendering.\n"
        ...
        "- Keep implementation production-grade, not placeholder/demo style.\n"
        "- No external generated image dependency.\n\n"
        f"Keyword: {keyword}\n"
        f"Title: {title}\n"
        f"Genre: {genre}\n"
        f"Objective: {objective}\n"
        ...
        "Implement the game faithfully to intent contract while keeping browser compatibility (no npm import dependency).\n\n"
        "Original HTML:\n"
        f"{html_content}"
    )
