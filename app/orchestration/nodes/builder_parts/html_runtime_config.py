...
def _normalize_mode_balance_config(*, core_loop_type: str, raw_config: dict[str, Any]) -> dict[str, Any]:
    ...
    if core_loop_type == "f1_formula_circuit_3d":
        config["player_hp"] = max(4, min(8, _coerce_int(config.get("player_hp"), fallback=6)))
        config["time_limit_sec"] = max(90, min(360, _coerce_int(config.get("time_limit_sec"), fallback=120)))
        config["enemy_spawn_rate"] = max(0.38, min(1.35, _coerce_float(config.get("enemy_spawn_rate"), fallback=0.62)))
        ...
        config["enemy_spawn_rate"] = max(0.28, min(1.2, _coerce_float(config.get("enemy_spawn_rate"), fallback=0.52)))
    elif core_loop_type in {"comic_action_brawler_3d", "duel_brawler"}:
        config["player_hp"] = max(7, min(12, _coerce_int(config.get("player_hp"), fallback=9)))
        config["time_limit_sec"] = max(80, min(260, _coerce_int(config.get("time_limit_sec"), fallback=120)))
        config["enemy_spawn_rate"] = max(0.2, min(1.2, _coerce_float(config.get("enemy_spawn_rate"), fallback=0.44)))
        config["enemy_speed_min"] = max(45, min(160, _coerce_int(config.get("enemy_speed_min"), fallback=68)))
        config["enemy_speed_max"] = max(80, min(220, _coerce_int(config.get("enemy_speed_max"), fallback=128)))
